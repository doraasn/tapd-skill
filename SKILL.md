...
10. **创建需求时分类（category_id）不可瞎猜**：金田铜业项目（52296479）已知分类：1252=项目需求、1254=品质、1256=**技术需求**、1257=优化需求。API 无权限直接查分类名（`categories` 系列接口 403），但可通过"已分好类的需求反查其 category_id"确认分类。创建新需求时先确认目标分类的 category_id 再创建，或创建后核对。教训："问题排查"曾先后被误分到 项目需求/优化需求
