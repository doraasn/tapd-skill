...
10. **创建需求时分类（category_id）不可瞎猜**：金田铜业项目（52296479）已知分类：1252=项目需求、1254=品质、1257=优化需求、技术需求=？（待确认）。API 无权限查分类名（`categories` 系列接口 403），创建新需求前先询问用户应分到哪个分类，或创建后让用户在 TAPD 页面确认并手动修正。切勿凭需求名称猜分类——"问题排查"曾先后被误分到 项目需求/优化需求，用户手动改到技术需求
